...
const WordBuffer = struct {
    ...
    pub fn init(allocator: std.mem.Allocator) !WordBuffer {
        return WordBuffer{ .quotes = try arrayList(isa.Word, allocator), .operands = try arrayList(isa.Operand, allocator), .addresses = try arrayList(isa.Address, allocator), .code = try arrayList(u8, allocator), .type_sig = t.Sig.init(0, 0) };
    }
    ...
    pub fn writeEntry(self: *@This(), entry: DictEntry) !void {
        ...
        switch (entry.impl) {
            ...
            .address => {
                try self.code.appendSlice(&[_]u8{ @enumToInt(isa.OpCode.call), @intCast(u8, entry.impl.address) });
    ...
    pub fn writeOperand(self: *@This(), operand: isa.Operand) !void {
        self.type_sig = self.type_sig.compose(t.Sig.init(0, 1));

        try self.code.appendSlice(&[_]u8{ @enumToInt(isa.OpCode.push), @intCast(u8, self.operands.items.len) });
...
const DictEntry = struct { type_sig: t.Sig, impl: union(DictEntryTag) { op_code: isa.OpCode, address: isa.Address } };

const Dict = std.StringHashMap(DictEntry);

// Some of these are dynamically typed for now.
const std_library = [_]std.meta.Tuple(&.{ []const u8, t.Sig, isa.OpCode }){
    .{ "+", t.Sig.init(2, 1), .add },
    .{ "-", t.Sig.init(2, 1), .sub },
    .{ "*", t.Sig.init(2, 1), .mul },
    .{ "/", t.Sig.init(2, 1), .div },
    .{ "dup", t.Sig.init(1, 2), .dup },
    .{ "drop", t.Sig.init(1, 0), .drop },
    .{ "bi@", t.Sig.init(3, 2), .bi_at },
};
...
const Interpreter = struct {
    ...
    pub fn init(allocator: std.mem.Allocator) !Interpreter {
        ...
        for (std_library) |sl| {
            const entry = DictEntry{ .type_sig = sl[1], .impl = .{ .op_code = sl[2] } };
    ...
    pub fn eval(self: *@This(), s: []const u8) ![]const u8 {
        ...
        while (tokens.next()) |token| {
            if (std.mem.eql(u8, token, ":")) {
                ...
                try self.dict.put(copied_name, .{ .type_sig = word_buffer.type_sig, .impl = .{ .address = addr } });

                try writer.print("{s} : {s}\n", .{ self.new_word_name, self.new_word_buffer.type_sig });
                ...
                try prev_word_buffer.quotes.append(try word_buffer.toUserWord());
...
fn test_eval(expected: []const u8, actual: []const u8) !void {
    ...
    return std.testing.expectEqualStrings(expected, try interpreter.eval(actual));
}
...
test "naming a value" {
    const input = .{ ": size 2 ;", "size", "5 size *" };

    const output = .{ "size : -> num", "2 : num", "2 10 : num num" };

    try test_multiline_eval(&output, &input);
}

test "further examples of defining a value" {
    const input = .{ ": pi 3.14159 ;", ": radius 10 ;", "radius radius * pi *", ": circumference 2 pi * radius * ;", "circumference" };

    const output = .{ "pi : -> num", "radius : -> num", "314.159 : num", "circumference : -> num", "314.159 62.8318 : num num" };

    try test_multiline_eval(&output, &input);
}

test "procedure definition" {
    const input = .{ ": square dup * ;", "21 square", "2 5 + square", "3 square square" };

    const output = .{ "square : num -> num", "441 : num", "441 49 : num num", "441 49 81 : num num num" };

    try test_multiline_eval(&output, &input);
}

// test "sum of squares" {
//     const input = .{ ": square dup * ;", ": sum-of-square [ square ] bi@ + ;", "3 4 sum-of-square" };

//     const output = .{ "square : num -> num", "sum-of-square : num num -> num", "25 : num" };
